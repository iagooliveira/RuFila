...
export class DivulgarCardapioService {
  ...
  cadastraPrato(
    ...
    return this.http.post<any>(
      `http://${environment.URL_PC_REMOTO}:3005/pratos`,
      {
        principal: principal,
        salada: salada,
        carne: guarnicao,
  ...
  listaPrato(codigoPrato: number): Observable<any> {
    return this.http.get(
      `http://${environment.URL_PC_REMOTO}:3005/pratos/${codigoPrato}`
    );
  }

  cadastraCardapio(restauranteCodigo: number, data: string): Observable<any> {
    return this.http.post<any>(
      `http://${environment.URL_PC_REMOTO}:3005/cardapios`,
      {
  ...
  cadastraPratoCardapio(
    cardapioCodigo: number,
    pratoCodigo: number
  ): Observable<any> {
    return this.http.post<any>(
      `http://${environment.URL_PC_REMOTO}:3005/cardapio_prato`,
      {
